...
import { getDeliveryStatuses, deleteDeliveryStatusById } from 'apiSdk/delivery-statuses';
import { DeliveryStatusInterface } from 'interfaces/delivery-status';

type ColumnType = ColumnDef<DeliveryStatusInterface, unknown>;

interface DeliveryStatusListPageProps {
...
export function DeliveryStatusListPage(props: DeliveryStatusListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DeliveryStatusInterface>>(
    () => `/delivery-statuses?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: DeliveryStatusInterface) => {
    if (hasAccess('delivery_status', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/delivery-statuses/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/delivery-statuses/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('delivery_status', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/delivery-statuses/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="delivery_status" />
        </Flex>

        {hasAccess('delivery_status', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/delivery-statuses/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
