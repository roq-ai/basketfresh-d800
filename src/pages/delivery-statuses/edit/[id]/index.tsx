...
import { getDeliveryStatusById, updateDeliveryStatusById } from 'apiSdk/delivery-statuses';
...
function DeliveryStatusEditPage() {
  const router = useRouter();
  const id = router.query.id as string;

  const { data, error, isLoading, mutate } = useSWR<DeliveryStatusInterface>(
    () => (id ? `/delivery-statuses/${id}` : null),
  ...
  const handleSubmit = async (values: DeliveryStatusInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateDeliveryStatusById(id, values);
      mutate(updated);
      resetForm();
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <Flex justifyContent={'flex-start'}>
            ...
            <Button
              ...
              gap="0.5rem"
              mr="4"
              onClick={() => router.push('/delivery-statuses')}
              _hover={{
                bg: 'neutral.transparent',
                color: 'neutral.main',
              }}
